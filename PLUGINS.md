...
- **共享单例**：宿主在运行时把 `React` / `ReactDOM` / `react/jsx-runtime` / `@ttool/sdk` 暴露为全局。插件构建时把它们标为 external，因此插件 bundle 不打包 React/SDK，**运行时复用宿主的同一份实例**（只有一份 React，hooks/context 才正常）。
...
2. **写工具**（`src/index.tsx`）：
   ...
   可用的 SDK 见下方「SDK API」。**只依赖 `@ttool/sdk`，不要自己打包 React。**
...
3. **构建配置**（`vite.config.ts`，库模式 IIFE + external→全局，见模板）：
   ...
   build: {
     ...
     rollupOptions: {
       ...
       output: { globals: { react: 'React', 'react-dom': 'ReactDOM', 'react/jsx-runtime': 'ReactJsxRuntime', '@ttool/sdk': 'TToolSDK' } },
     },
   }
   ```
...
- **不要打包 React / @ttool/sdk**：必须按上面的 external 配置，否则会出现多份 React 导致 hooks 崩溃。
