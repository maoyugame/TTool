...
`image-tool` 是 renderer-only 的内置批量图片工具，使用浏览器 File / Image / Canvas / Object URL 能力完成处理，不调用 Electron API、不上传源文件，也不扩展外部插件 SDK。

- 支持文件选择与拖放一次导入多张常见栅格图片，队列显示文件名、格式、尺寸、体积和状态，并允许选择、移除或清空；动态 GIF 等动画输入只处理静态首帧。
- 单批最多 20 个文件、合计 200 MiB；每个输入上限为 50 MiB、单边 16384 px，单张输出画布上限为 4000 万像素。损坏或不支持的项目必须独立失败，不能阻塞其他项目或让工具页白屏。
- 尺寸模式包括保持原图、百分比、最长边（不放大）和精确宽高；可统一执行 90° 旋转、水平翻转、垂直翻转，并在当前选中项目上预览。
- 输出格式为 PNG、JPEG、WebP；JPEG/WebP 提供质量压缩和 JPEG 透明背景填充，PNG 采用无损重新编码并明确提示体积可能不降反升。
- 批处理按顺序执行，显示总体进度、逐项结果、实际输出体积和节省比例；成功结果既可单独下载，也可打包为自动处理重名的标准 ZIP。
- 原文件始终保持不变；图片、结果、下载和压缩包的对象 URL、解码资源及过期异步任务应在替换或组件卸载时清理。SSR 初始渲染不得访问 `window`、`document`、`Image` 或其他仅浏览器存在的全局对象。
- 当前版本不承诺保留动画、EXIF/ICC 等元数据，也不包含裁剪、修图、标注、图层、目标体积压缩、文件夹监听或桌面原生编码器；这些能力需作为独立范围评估。
