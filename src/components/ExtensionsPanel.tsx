          ...
          <>
            ...
            {devMode ? (
              <div style={{ marginBottom: 22 }}>
                <span onClick={installLocal} style={{ fontSize: 12.5, fontWeight: 540, color: 'var(--accent)', cursor: 'pointer' }}>＋ 从本地文件夹安装（开发者模式）</span>
              </div>
            ) : (
              <div style={{ marginBottom: 22, fontSize: 12, color: 'var(--text3)' }}>在设置中开启「开发者模式」可从本地文件夹安装用于自测。</div>
            )}
                    ...
                    <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 13px', borderRadius: 13, background: 'var(--surface2)', border: '1px solid var(--hair)', opacity: p.enabled ? 1 : 0.55 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--text2)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {isGithub ? '↗ ' + (p.source as { repo: string }).repo : p.source?.type === 'local' ? '本地' : ''} · {m.desc || m.cat}
                        </div>
                      </div>
