...
packages/sdk/       @maoyugames/ttool-sdk 包（类型 + 运行时桥接）
...
4. **`@maoyugames/ttool-sdk` 发布在 npm 公共 registry**（源码 `packages/sdk/`，`prepublishOnly` 编译 dist）。SDK 表面变更后：bump `packages/sdk/package.json` 版本（破坏性变更同时 bump `SDK_VERSION` 主版本）→ `cd packages/sdk && npm publish --access public`（需维护者 npm 账号）→ 同步 `TTOOL-PLUGIN-GUIDE.md`。`packages/sdk/src/index.ts` 的类型必须与指南 §4 及 §11 兜底 shim 三处一致。
