...
`image-tool` 是完全在本机渲染进程内运行的内置工具。可通过文件选择或拖放导入常见栅格图片，按比例或自由修改宽高，执行 90° 旋转与水平/垂直翻转，并导出 PNG、JPEG 或 WebP；JPEG/WebP 可调质量，JPEG 可选择透明区域的填充背景。

- 原文件不会被覆盖或上传；处理结果在预览确认后以新文件下载。
- 输入限制为 50 MiB、单边不超过 16384 px，输出不超过 4000 万像素，避免超大画布拖垮渲染进程。
- 动态 GIF 等动画输入按静态首帧处理；当前版本不保留 EXIF/ICC 等元数据，也不提供裁剪、批处理或标注。
- 临时对象 URL 会在替换图片和离开工具页时释放；功能同时适用于 Web 与 Electron 渲染层，不依赖新的原生桥接或生产依赖。
