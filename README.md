...
即时翻译（接 MyMemory，实时互译）· JSON 格式化（校验 / 美化 / 压缩 / 语法高亮）· Unix 时间戳（双向转换）。全部为真实可用的实时计算，非静态图。图标在 `src/assets/icons/` 下（macOS 风格 app 图标）。
...
外部插件系统：受信任动态加载 + 桌面优先；插件构建成 IIFE bundle（react/SDK 外置→复用宿主单例）+ `manifest.json`，宿主按需懒加载、可启停/更新/卸载。SDK 以 npm 包 `@maoyugame/ttool-sdk` 分发（源码 `packages/sdk/`，发布前 `prepublishOnly` 编译出 dist），示例插件见 `examples/hello-tool/`。
