...
即时翻译（接 MyMemory，实时互译）· JSON 格式化（校验 / 美化 / 压缩 / 语法高亮）· Unix 时间戳（双向转换）· 截图贴图（桌面端截图 / 标注 / 置顶贴图）· Codex 用量状态（桌面端本机限额与 token 趋势）。全部为真实可用的实时计算，非静态图。图标在 `src/assets/icons/` 下（macOS 风格 app 图标）。
...
- 开始框选前会抓取各显示器的冻结画面；选区、后续标注和贴图都基于该帧，不会在完成选择时再次抓屏。
